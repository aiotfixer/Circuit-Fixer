...
👉 [https://aiotfixer.github.io/Circuit-Fixer](https://aiotfixer.github.io/Circuit-Fixer/)
---
### 🌟 遊戲特色 (Feature)
  ...
  - 免安裝跨平台：純前端單檔 (Single-file) 開發，完美支援手機、平板觸控，以及電腦滑鼠點擊，開啟網頁即可遊玩。
...
### 💯 分數計算機制 (Scoring System)
  ...
  為了鼓勵玩家「既要快，又要精準」，單局得分由以下三個項目加總而成，關卡越高，加分倍率越大！
  
  - 過關基礎分：通關層數 (Level) × 100。只要過關必定獲得的底分。
  
  - 速度加分 (Time Bonus)：系統標準挑戰時間為 30 秒。若在 30 秒內過關，每省下 1 秒鐘，可獲得 10 × 關卡數 的加分。
  
  - 步數加分 (Click Bonus)：系統會依據當下棋盤大小給予容錯步數（總格數的 3 倍）。若點擊次數低於容錯步數，每少點 1 次，可獲得 5 × 關卡數 的加分。
